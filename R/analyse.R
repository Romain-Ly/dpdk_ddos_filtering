...
pdf(file=args[4],useDingbats=FALSE) 
	with(plot.df,plot(rx.dt~t,type="l",ylim=c(0,ceiling(ymax)),lwd=tck,bty="l",xlim=c(0,60),xlab="Temps (secondes)",ylab="Packets par seconde"))
	legend("topright",inset=c(0,0.05), title ="Paquets",c("Rx","Tx","Drop"),
		lty=c(1,1,1),lwd=c(1,1,1),col=c("red","green","blue"),
		bty="o"
)     

	with(plot.df,lines(tx.dt~t,type="l",col="green",lwd=tck))
	with(plot.df,lines(flt.dt~t,type="l",col="red",lwd=tck))
	with(plot.df,lines(drop.dt~t,type="l",col="blue",lwd=tck))
dev.off()
